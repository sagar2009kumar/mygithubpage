...
public function ws_currentDeals($store_id, $service){
	...
	public function getProductDetailDescription($storeId, $productId, $currentCurrencyCode) {
		...
		try {
			...
			$res = array("status"=>"1","stockQty"=>(string)$stockQty,"sku"=>(string)$productSku, "name"=>$productName, "categories"=>$productCategoriesIds,
						...
						"productId"=> (string)$productId, "imageUrl"=> (string)$productImageUrl, "productRating"=> (string)$ratingPercent,"customAttribute"=> $customAttrArr,"customAttributeCount"=> (string)$customAttributeCount, "allVisibleAttributes"=> $attributeArr,"visibleAttributeCount"=>(string)$attrCount);
   ...
   /**** Cart Sync written by sagar but nothing in comparison to Lakshya Sir ****/
	...
	public function getCartItems($storeId, $quoteId = null, $customerId = null, $currency = 'USD', $updateCartMessages = null, $isInitialize = 0) {
		...
		//~ if($isInitialize) {
			//~ /**** is cart is going to be initialized ****/
		...
		if($noOfCartItem > 0) {
			...
			foreach($totals as $total) {
				$value = number_format($this->convert_currency($total->getValue(), self::$_baseCurrencyCode, $currency), 2, '.', '');
				if($total->getCode() == "grand_total") {
					$tempGrandTotal = $value;
					array_push($tempPriceArr, array("title"=>"GRAND TOTAL INCL. TAX","value"=>$value));
				}else {
					array_push($tempPriceArr, array("title"=>$total->getTitle(),"value"=>$value));
				}
				if($total->getCode() == "tax") {
					$tempTaxFlag = 1;
					$tempTax = $total->getValue();
				}
			}
			
			array_push($tempPriceArr, array("title"=>"GRAND TOTAL EXCL. TAX", "value"=>(string)($tempGrandTotal-$tempTax)));
	...
	public function initializeCheckout($storeId, $quoteId = null, $currency = 'USD') {
		...
		/**** IMPORTANT Remaining to set the default address before starting the order ****/
		...
		try {
			...
			if($size) {
				...
				foreach($_products as $product) {
					...
					//~ $reviewArr = array("reviewContent"=>$reviews->getContent(), "productRating"=>$productRating);
